...
module Minitest
  def self.start(options)
    @run = Fiber.new { run(options) }
    resume(:process)
  end

  def self.resume(frame)
    @frame = frame
    answer = @run.resume(frame)
    answer unless @run.alive?
  end
  ...
  module Waits
    ...
    def wait_process_frames(frames)
      at_least_one("wait_process_frames", frames).times { yield_until(:process) }
    end

    def wait_physics_frames(frames)
      at_least_one("wait_physics_frames", frames).times { yield_until(:physics) }
    end

    def wait_seconds(time)
      waited = 0.0
      waited += yield_physics_frame while waited < time
    end
    ...
    def begin_at_process_frame
      yield_until(:process) unless Minitest.frame == :process
    end
