...
| When | the scene runs |
| Then | the text given to `puts` appears in Godot's output |
...
| When | the scene runs |
| Then | the text given to `puts` appears in Godot's output once |

## `RS-003` `print` writes to Godot's output

| Step | Statement |
| --- | --- |
| Given | a node whose script is a `.rb` file calling `print` |
| When | the scene runs |
| Then | the text given to `print` appears in Godot's output |

## `RS-004` `p` writes what it is given, inspected, to Godot's output
